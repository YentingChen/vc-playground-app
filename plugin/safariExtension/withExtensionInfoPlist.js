...
const withExtensionInfoPlist = (config, { folderName }) => {
    return (0, config_plugins_1.withInfoPlist)(config, async (config) => {
        ...
        const infoPlistExists = fs.existsSync(path.join(extensionRootPath, "Info.plist"));
        if (infoPlistExists)
            return config;
        ...
        fs.writeFileSync(extensionFilePath, plist_1.default.build(extensionPlist));
        return config;
    });
};
